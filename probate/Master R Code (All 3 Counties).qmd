...
NOTE: You could probably tidy things up by iterating over the demographic variables, and applying the same function, since you're doing the same thing over and over again. Like this: 
...
age_distribution <- master2015 |>
  # NOTE: Looks good. I would probably filter out the NAs, but it depends on what you want the result to mean. 
...
NOTE: Not really ... you'll have to decide how to handle these situations. Like what the different patterns of NAs mean. If you can safely impute 0 for every NA that would be great. If not, can / should you just remove the rows with at least 1 NA? All 4 NA? A particular pattern of NAs? If you're doing inferential statistics downstream, it can be better to impute values than just delete, but how you do that is a whole topic. There appear to be a lot of cases where all values are NA (see analysis below). 
...
```{r}
# NOTE: look at distribution of how many NAs, but the pattern across columns is important too. 
master2015 |>
  select(No_Children, No_Dec_Child, No_Step_Child, No_Step_Dec) |> #View()
  rowwise() |>
  mutate(na_count = sum(is.na(c_across(everything())))) |> #View()
  group_by(na_count) |>
  summarize(count = n()) |> 
...
NOTE: Same as above here, you can iterate over the column names and apply the same function to each grouping variable of interest. 
## Gender
...
sex_probate_summary <- master2015 |>
  ...
  group_by(Sex) |>
  summarize(
    # NOTE: i think you have 0 NAs in this column, but if your sum function doesn't handle the condition where there are. 
    # NOTEL: you can drop the "== 1" here, since Probate_Estate is already TRUE/FALSE you can just sum it directly. 
    probate_count = sum(Probate_Estate == 1),
    # NOTE: i tested this and it just counts the number of records in the group, including those with NAs. Notice you don't "focus" on Probate_Estate column here. But you have no NAs, so i think it's fine.  
