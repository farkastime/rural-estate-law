...
rlf_dd <- rlf %>%
    select( # select and rename
        ...
        AGE = R15AGEY_E, GENDER = RAGENDER, RACE = RARACEM, HISP = RAHISPAN, 
        REGION = R15CENREG, RURAL = R15URBRUR, EDUC_YRS = RAEDYRS,
...
dd_mod <- dd %>%
    ...
    drop_na() # don't do this
dd_srvy_mod <- as_survey(dd_mod, strata = STRATUM, weights = WGHT_R, id = SECU, nest = TRUE)
mod1 <- svyglm(WILL_IND ~ RURAL + AGE + WEALTH + RACE + HISP + GENDER + HLTH_CHNG, design = dd_srvy_mod, family = 'quasibinomial')
summary(mod1)
mod_gen <- svyglm(WILL_IND ~ GENDER, design = dd_srvy_mod, family = 'quasibinomial')
summary(mod_gen)
```