...
# pivot long
dd %>%
    pivot_longer(c(starts_with('SPOUSE_ID'), starts_with('HHID')))
```
```{r}
rlf_dd <- rlf %>%
    select( # select and rename
        ...
        HHID = paste0('H', wave, 'HHIDC'), # household id
        INW15, # flag for wave 15 
        STRATUM = RAESTRAT, # sampling stratum  
        SECU = RAEHSAMP, # sampling cluster (nested within stratum) 
        WGHT_R = paste0('R', wave, 'WTRESP'), # respondent weight
        WGHT_H = paste0('R', wave, 'WTHH'), # respondent weight
        AGE = paste0('R', wave, 'AGEY_E') # age in years
