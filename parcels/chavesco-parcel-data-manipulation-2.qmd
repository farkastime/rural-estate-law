...
```{r}
dd <- dd_raw %>%
  drop_na(CLASSCD1_DESC) %>% 
  separate_rows(NAME, sep = ";") %>% 
  filter(
    !str_detect(NAME, biz_patterns), 
    str_detect(NAME, ","), 
    ) %>%
  separate_wider_delim(
    ...
    names = c("LAST", "FIRST"), 
    too_many = "drop")  %>% 
  separate_rows(FIRST, sep = "&") %>% 
  mutate(
    across(FIRST, ~str_trim(.x)),
    first_no_suffix = str_extract(FIRST, paste0("^.*(?=", suffix_pattern, ")")), 
    SUFFIX = str_extract(FIRST, suffix_pattern)
    ) %>% 
  mutate(across(FIRST, ~ifelse(is.na(first_no_suffix), .x, first_no_suffix))) 
```
...
parsed <- parsed %>%
  mutate(
    ...
    #   NA_character_
    # ),
    suffix = str_extract(NAME, suffix_pattern) %>%
      str_trim, 
    # given_names = str_trim(str_remove(remainder, suffix_pattern))
  )

parsed %>% select(NAME, last_name, suffix) %>% print(n = 1000)
